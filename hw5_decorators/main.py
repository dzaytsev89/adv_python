from decor import logger, logger_to_spec_file
import bs4
import requests
import re
...
def search_keys(keys):
    ...
    for article in articles:
        ...
        for k in keys:
            if search_word(k.lower(), title_data, post_preview.text, post_text, *hubs):
...
def main():
    while True:
        keywords = input('Введите значения для поиска через пробел: ').split()
        if 'exit' in keywords:
            exit()
        pprint(search_keys(keywords))
